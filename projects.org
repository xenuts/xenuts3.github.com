...
| [[./img/tennis-300x183.png]] | A two layered data association method to improve the robustness of tennis ball tracking. At the local layer, a shift token transfer method is proposed, based on shift window processing, to generate a set of short trajectories or ``trajectorylets''. At the global layer, a unique ball trajectory is obtained by applying a dynamic programming based splice method to a directed acyclic graph consisting of trajectorylets. |
...
| [[./img/atm-300x183.png]] | We apply a real-time object tracking approach based on a stereo camera placed in front of ATM machines. With the aid of camera’s real-time monitoring, tracking and counting, a queue-aware system is implemented to provide each arriving customer a suggested queue number and estimated corresponding queuing time. |
